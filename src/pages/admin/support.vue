              ...
              <q-menu auto-close>
                <q-list dense separator style="min-width: 170px">
                  <q-item clickable  @click="editRow(props.row)">
                    ...
                    <q-item-section>{{$t('support.Edit')}}</q-item-section>
                  </q-item>
                  <q-item clickable @click="props.row.deletePopup = true">
                    <q-item-section avatar>
                      <q-icon class="icon" name="delete" />
                    </q-item-section>
                    <q-item-section>{{$t('support.Delete')}}</q-item-section>
                  </q-item>
                </q-list>
              </q-menu>
            </q-btn>
            <q-dialog v-model="props.row.editPopup" persistent>
              <q-card class="detail-dialog">
                <q-card-section class="detail-title">
                  {{$t('support.EditSupport')}}
                </q-card-section>
                <q-card-section>
                  <div>
                      <q-input outlined v-model="edit.Name" :label="$t('support.Name')" readonly></q-input>
                  </div>
                  <div>
                      <q-input outlined v-model="edit.Phone" :label="$t('support.Phome')" readonly></q-input>
                  </div>
                  <div>
                      <q-input outlined v-model="edit.Email" :label="$t('support.Email')" readonly></q-input>
                  </div>
                  <div class="q-mt-md">
                      <q-input outlined type="textarea" v-model="edit.Message" :label="$t('support.Message')"> </q-input>
                  </div>
                </q-card-section>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'Name', label: this.$t('support.Name'), field: 'Name', align: 'left' },
        { name: 'Email', label: this.$t('support.Email'), field: 'Email', align: 'left' },
        { name: 'Phone', label: this.$t('support.Phone'), field: 'Phone', align: 'left' },
        { name: 'Date', label: this.$t('support.Date'), field: 'CreateDate', align: 'left' },
  ...
  methods: {
    loadData (props) {
        ...
        let _this = this
        // get list
        this.$api.post('support/search.php', {
            Name: this.filter,
            OrderBy: sortBy,
    ...
    deleteRow (id) {
        let _this = this
        this.$api.post('support/delete.php', {
            ID: id
        }).then(function (response) {
    ...
    async updateRow (row) {
        let _this = this
        this.$api.post('support/update.php', this.edit).then(function (response) {
          _this.loadData({pagination: _this.pagination, filter: _this.filter})
        }).catch(function (error) {
            _this.$helper.logError(error)
            _this.$q.notify({

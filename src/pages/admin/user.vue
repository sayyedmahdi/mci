<template>
  <div class="tw-flex tw-flex-col-reverse md:tw-flex-row md:tw-space-x-10 md:tw-justify-between tw-p-6 tw-mt-24">
    ...
    <div class="box_shadow tw-flex tw-mb-16 md:tw-mb-0 tw-flex-col tw-space-y-5 tw-justify-center tw-items-center text_dark_blue md:tw-w-[350px]">
      ...
      <div class="text_dark_pink xs:tw-text-2xl">{{user.Username}}</div>
...
export default {
  ...
  methods: {
    ...
    getUser(){
      ...
      this.$api.post('user/get.php?id=' + this.$route.params.id, null)
        .then((res) => {
          this.user = res.data
        })
        .catch((err) => {
    ...
    userInfo(){
      this.$api.post('user/info.php?id=' + this.$route.params.id, null)
        .then((res) => {
          this.info = res.data
        })
        .catch((err) => {
